...
#define LUA_MEM_EXTRA_MAX \
  (0)  // max allowed memory usage for Lua bitmaps (in bytes)
#define LUA_MEM_MAX \
...
bool IS_EXTERNAL_MODULE_ON(void);
    ...
    (!defined(PCBX9LITE) || defined(PCBX9LITES))
#define HARDWARE_INTERNAL_RAS
#endif

void intmoduleSerialStart(uint32_t baudrate, uint8_t rxEnable, uint16_t parity,
                          uint16_t stopBits, uint16_t wordLength);
...
#define TRAINER_CONNECTED()                                               \
  (GPIO_ReadInputDataBit(TRAINER_DETECT_GPIO, TRAINER_DETECT_GPIO_PIN) == \
   TRAINER_DETECT_GPIO_PIN_VALUE)
...
#define KEYS_GPIO_REG_LEFT
#define KEYS_GPIO_REG_RIGHT
...
#define DEFAULT_SWITCH_CONFIG                                        \
  (SWITCH_NONE << 14) + (SWITCH_NONE << 12) + (SWITCH_NONE << 10) +  \
      (SWITCH_NONE << 8) + (SWITCH_2POS << 6) + (SWITCH_3POS << 4) + \
      (SWITCH_2POS << 2) + (SWITCH_2POS << 0)
#define DEFAULT_POTS_CONFIG \
  (POT_WITHOUT_DETENT << 0) + (POT_WITHOUT_DETENT << 2);
...
extern volatile uint32_t
    pwm_interrupt_count;  // TODO => reusable buffer (boot section)
#else
...
#if defined(PCBX9D)
#define IS_POT(x) \
  ((x) >= POT_FIRST && (x) <= POT2)  // POT3 is only defined in software
#else
...
#define UNEXPECTED_SHUTDOWN() \
  (WAS_RESET_BY_WATCHDOG() || g_eeGeneral.unexpectedShutdown)
#endif
...
#define IS_SPORT_UPDATE_POWER_ON()                                            \
  (GPIO_ReadInputDataBit(SPORT_UPDATE_PWR_GPIO, SPORT_UPDATE_PWR_GPIO_PIN) == \
   Bit_SET)
#else
...
#if defined __cplusplus
void auxSerialSetup(unsigned int baudrate, bool dma, uint16_t length = 8,
                    uint16_t parity = 0, uint16_t stop = 0);  // TODO-feather
#endif
void auxSerialInit(unsigned int mode, unsigned int protocol);
void auxSerialPutc(char c);
#define auxSerialTelemetryInit(protocol) \
  auxSerialInit(UART_MODE_TELEMETRY, protocol)
...
#if defined(RADIO_TX12) || defined(RADIO_TPRO) || \
