    ...
    switch (rp->type) {
      case ACK:
        if (rp->idx == packet.idx && rp->crc == packet.crc) {
          ...
          ESP_LOGE(TAG, "Ack failed: idx: %d vs %d, crc: %d vs %d, ", rp->idx,
                   packet.idx, rp->crc, packet.crc);
        }
        break;
      case TELE:
...
inline void process_bind(Event_t &evt)
{
  ...
  if (BIND == rp->type) {
    ...
    if (crc == rp->crc) {
      ...
      memcpy(g_model.moduleData[INTERNAL_MODULE].espnow.rx_mac_addr,
             rxPeer.peer_addr, ESP_NOW_ETH_ALEN);
      ...
      esp_wifi_set_channel(g_model.moduleData[INTERNAL_MODULE].espnow.ch,
                           (wifi_second_chan_t)0);
      txState = PULSES;
    } else {
...
static void tx_task(void *pvParameter)
{
  ...
  while (pulsesON) {
    if (xQueueReceive(evtQueue, &evt, TX_PERIOD_MS / portTICK_RATE_MS) ==
        pdTRUE) {
      switch (evt.id) {
        case TX:
      ...
      switch (txState) {
        case PULSES: {
          ...
          if (linkState != QUEUED) {
            ...
            esp_err_t ret = esp_now_send(
                rxPeer.peer_addr, (const uint8_t *)&packet, sizeof(packet));
            if (ret != ESP_OK) {
        ...
        case BINDING:
          if (linkState != QUEUED) {
            ...
            bind_packet_prepare();
            esp_err_t ret = esp_now_send(
                broadcast_mac, (const uint8_t *)&packet, sizeof(packet));
            if (ret != ESP_OK) {
              ESP_LOGE(TAG, "Send txPacket error: %s", esp_err_to_name(ret));
            } else {
              linkState = QUEUED;
...
esp_err_t initTX()
{
  ...
  rxPeer.channel = g_model.moduleData[INTERNAL_MODULE].espnow.ch;
  memcpy(rxPeer.peer_addr,
         g_model.moduleData[INTERNAL_MODULE].espnow.rx_mac_addr,
         ESP_NOW_ETH_ALEN);
  ESP_ERROR_CHECK_WITHOUT_ABORT(esp_now_add_peer(&rxPeer));
  ...
  txState = PULSES;
  if (NULL ==
      xTaskCreateStaticPinnedToCore(tx_task, "tx_task", ESPNOW_STACK_SIZE, NULL,
                                    ESP_TASK_PRIO_MAX - 6, espnow_stack,
                                    &espnowTaskBuffer, PULSES_TASK_CORE)) {
...
bool is_binding_espnow() { return BINDING == txState; }
...
static void espNowDeInit(void *context) { pulsesON = false; }

static void espNowSetupPulses(void *context, int16_t *channels,
                              uint8_t nChannels)
{
  // nothing to do
}
...
static void espNowProcessData(void *context, uint8_t data, uint8_t *buffer,
                              uint8_t *len)
{
#if 0
