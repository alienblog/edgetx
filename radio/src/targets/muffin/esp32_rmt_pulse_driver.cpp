...
static void esp32_rmt_rx_task(void *pdata)
{
  ...
  while (!ctx->exit) {
    // ready to receive
    ESP_ERROR_CHECK(
        rmt_receive(ctx->rmt, raw_symbols, sizeof(raw_symbols), &ctx->rx_cfg));
...
rmt_ctx_t *esp32_rmt_tx_init(rmt_ctx_t *ctxmem, int pin,
                             rmt_reserve_memsize_t memsize, float tick_in_ns,
                             rmt_tx_encode_cb_t enc_fn, size_t pulse_in_frame)
{
...
rmt_ctx_t *esp32_rmt_rx_init(rmt_ctx_t *ctxmem, int pin,
                             rmt_reserve_memsize_t memsize, float tick_in_ns,
                             rmt_rx_decode_cb_t dec_fn, size_t pulse_in_frame,
                             ...
                             size_t min_pulse_in_ns)
{
  ...
  if (NULL != rvalue) {
    rmt_rx_channel_config_t rx_channel_cfg = {
        ...
        .mem_block_symbols = memsize,  // amount of RMT symbols that the channel
                                       // can store at a time
    };
    ...
    if ((NULL != rvalue->rmt) && (NULL != rvalue->rxQueue)) {
      ...
      xTaskCreateStaticPinnedToCore(
          esp32_rmt_rx_task, "esp32_rmt_rx_task",
          sizeof(rvalue->rmt_task_stack) / sizeof(rvalue->rmt_task_stack[0]),
          rvalue, 5, rvalue->rmt_task_stack, rvalue->task_struct,
          TRAINER_PPM_OUT_TASK_CORE);  // TODO-ESP32 priority
      rmt_rx_event_callbacks_t cbs = {
          .on_recv_done = rmt_rx_done_callback,
      };
      ESP_ERROR_CHECK(
          rmt_rx_register_event_callbacks(rvalue->rmt, &cbs, rvalue->rxQueue));
    } else {
      free(rvalue);
      rvalue = NULL;
    }
...
void esp32_rmt_stop(rmt_ctx_t *ctx)
{
  ...
  if (NULL != ctx->rxQueue) {
    ...
    xQueueSend(ctx->rxQueue, &end,
               0);  // doesn't matter what was sent, just send to end the loop
  }
  // TODO-v5: no thing special need to be done for TX. It will end after the
  // last frame sends out
}
...
int rmt_ppm_decode_cb(rmt_ctx_t *ctx, rmt_symbol_word_t *rxdata,
                      size_t rxdata_len, int16_t *ppm_decode_buf)
{
  int channel = 0;
  uint32_t high = 0U;
  for (size_t i = 0; i < rxdata_len; i++) {
    if (i != 0) {
      uint32_t val = (high + rxdata[i].duration0) *
                     (ctx->tick_in_ns / 1000);  // result in us
      if (val > 800 && val < 2200) {
        ppm_decode_buf[channel++] = (int16_t)(val - 1500) *
                                    (g_eeGeneral.PPM_Multiplier + 10) /
                                    10;  // +-500 != 512, but close enough.
      } else {
        channel = -1;
        break;
      }
